...
import type { PurchaserInfo, CreditCardInfo, NetOrderRequest } from "../types";
...
export default function PurchaserDashboard() {
  ...
  // クレジットカード情報の更新
  const updateCreditCardInfo = (field: keyof CreditCardInfo, value: string) => {
  ...
  const handleOrder = async () => {
    ...
    const { cardNumber, holderName, expiryDate, securityCode } = creditCardInfo;
    ...
    try {
      ...
      const orderRequest: NetOrderRequest = {
        productDetails,
        purchaserInfo,
        creditCardInfo,
        shippingFee: 660,
      ...
      const response = await fetch(`${apiUrl}/api/orders/net`, {
        method: "POST",
      ...
      if (data.status === "success") {
        showNotification("注文が完了しました。ご利用ありがとうございました。", "success");
        clearCart();
        fetchProducts();
        // フォームをリセット
        ...
        showNotification(data.message ?? "注文処理中にエラーが発生しました。", "error");
      }
    } catch {
  ...
  return (
    <DashboardLayout>
      ...
      >
        ...
        <Box width="100%" maxWidth={{ xs: "100%", md: 1200 }} mt={3} mb={1}>
          <Typography align="left">購入者情報・クレジットカード情報</Typography>
        </Box>
        <Box
          display="flex"
          flexDirection="column"
        ...
        >
          ...
          <TextField
            ...
            onChange={(e) => updateCreditCardInfo("holderName", e.target.value)}
            size="small"
            required
            fullWidth
          />
          ...
          <TextField
            ...
            onChange={(e) => updateCreditCardInfo("securityCode", e.target.value)}
            size="small"
            required
            fullWidth
          />
